// Roll Num : MT2024137
// Name : Shah Mohit P
// 1. Write a separate program (for each time domain) to set a interval timer in 10sec and
// 10micro second
// a. ITIMER_REAL
// b. ITIMER_VIRTUAL
// c. ITIMER_PROF

#include<stdio.h>
#include<sys/time.h>
#include<unistd.h>
#include<signal.h>
#include<stdlib.h>

void signal_output(int sig){
    printf("Signal Received\n");
}

int main(){
    
    struct itimerval timer;

    signal(SIGVTALRM, signal_output);
    perror("Status:");

    timer.it_interval.tv_sec = 1; //After first every signal would come in 10 sec
    timer.it_interval.tv_usec = 10;
    timer.it_value.tv_sec = 1; //First Signal in 10 sec
    timer.it_value.tv_usec = 10;

    setitimer(ITIMER_VIRTUAL, &timer, NULL); //IN virtual only the kernel time is taken into consideration

    while(1){

    }
    
}

//OUTPUT
// mohit@mohit-14IRL8:~/SSHandsOnList2$ cc 1b.c
// mohit@mohit-14IRL8:~/SSHandsOnList2$ ./a.out
// Status:: Success
// Signal Received
// Signal Received
// Signal Received
// Signal Received
// Signal Received
// Signal Received
// ^C
